...
function updateGeneratorForm(lang) {
  ...
  const iframeSrc = "https://script.google.com/macros/s/AKfycby3M6gd0ycvwbi015iLckIJckCigRHWyojEQ1K8KVjDP-3mL8LtaicVyYEYpmjDJNJ7GA/exec";
...
function translatePage() {
  ...
  // HTML-переводы: полная замена innerHTML
  ...
  // Текстовые переводы: замена текстовых узлов с сохранением дочерних элементов
  document.querySelectorAll('[data-i18n]').forEach(element => {
    ...
    if (!hasElementChildren) {
      ...
      // Есть дочерние элементы (стрелки ▾▸ и др.) — заменяем только первый текстовый узел
