...
const translations = {
  "ru": {
    ...
    "banner-desc": "Если при загрузке информатора появляется ошибка 403 — воспользуйтесь локальной версией. Она работает прямо в браузере без подключения к серверу Google.",
  ...
  "en": {
    ...
    "banner-desc": "If the announcement editor fails to load with a 403 error, switch to the local version. It works directly in your browser without connecting to Google servers.",
...
function updateGeneratorForm(lang) {
  ...
  const mode = localStorage.getItem('announcementGeneratorMode') || 'server';
  if (mode === 'local') {
    renderLocalAnnouncement(lang);
    return;
  }

  let iframeSrc = "https://script.google.com/macros/s/AKfycby0yF_RA9qxS7yjr0CqqQ0-Ln55OYlgEmctY9BY3w9T93yhl5VcmRETHtgQswMByIVXaA/exec";
  if (lang === 'en') {
    iframeSrc = "https://script.google.com/macros/s/AKfycbzTpBcYlFKXGaAhB5AFW0L5OCq7u_uJ680ii2MB8ljWXy_sI88b-WO0ZbOaIYpaI7ffzg/exec";
  }
  formWrapper.innerHTML = `<iframe class="announcement-frame announcement-frame--server" src="${iframeSrc}" frameborder="0" marginheight="0" marginwidth="0" style="width:100%;min-height:2600px;height:2600px;border:none;display:block;"></iframe>`;
}

function getLocalAnnouncementFrameSrc(lang) {
  const currentLang = lang || (localStorage.getItem('language') || 'ru');
  return `local-generator.html?lang=${encodeURIComponent(currentLang)}`;
}

function renderLocalAnnouncement(lang) {
  const formWrapper = document.getElementById('form-wrapper');
  if (!formWrapper) return;
  const theme = localStorage.getItem('theme') || 'light';
  const iframeBackground = theme === 'dark' ? '#0a0a0a' : '#f3f4f6';
  formWrapper.style.backgroundColor = iframeBackground;
  formWrapper.innerHTML = `
    <div style="width:100%;min-height:100vh;background:${iframeBackground};">
      <iframe
        id="announcement-local-frame"
        class="announcement-frame announcement-frame--local"
        src="${getLocalAnnouncementFrameSrc(lang)}"
        title="Announcement local generator"
        style="width:100%;min-height:1200px;height:1200px;border:none;display:block;background:${iframeBackground};"
        sandbox="allow-scripts allow-forms allow-same-origin allow-modals allow-downloads"
      ></iframe>
    </div>`;
}

function getLocalAnnouncementHTML(lang) {
  const isEn = lang === 'en';
  const ui = {
    title: isEn ? 'Announcement Editor' : 'Локальный редактор информатора',
    subtitle: isEn
      ? 'Create announcement JSON locally in your browser. This version does not depend on Google Apps Script.'
      : 'Создавайте JSON для информатора локально в браузере. Эта версия не зависит от Google Apps Script.',
    hint: isEn
      ? 'Format: "Announcement name": SoundId. To delete an announcement, set SoundId to 0.'
      : 'Формат: "Название информатора": SoundId. Чтобы удалить информатор, установите SoundId = 0.',
    name: isEn ? 'Announcement name' : 'Название информатора',
    sound: 'SoundId',
    addRow: isEn ? 'Add row' : 'Добавить строку',
    example: isEn ? 'Load example' : 'Загрузить пример',
    clear: isEn ? 'Clear' : 'Очистить',
    generate: isEn ? 'Generate JSON' : 'Сгенерировать JSON',
    copy: isEn ? 'Copy JSON' : 'Скопировать JSON',
    output: isEn ? 'Generated JSON' : 'Сгенерированный JSON',
    rowRemove: isEn ? 'Delete' : 'Удалить',
    emptyState: isEn ? 'Add at least one announcement row.' : 'Добавьте хотя бы одну строку информатора.',
    invalidState: isEn ? 'Fill in the announcement name and SoundId correctly.' : 'Корректно заполните название информатора и SoundId.',
    copied: isEn ? 'JSON copied to clipboard.' : 'JSON скопирован в буфер обмена.',
    copyError: isEn ? 'Could not copy JSON automatically.' : 'Не удалось автоматически скопировать JSON.',
    namePlaceholder: isEn ? 'For example: yellow avenue' : 'Например: yellow avenue',
    soundPlaceholder: isEn ? 'For example: 12' : 'Например: 12'
  };
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    body { margin: 0; font-family: Arial, sans-serif; background: #0f1115; color: #e5e7eb; padding: 24px; }
    .wrap { max-width: 1100px; margin: 0 auto; display: grid; gap: 20px; }
    .card { background: #171a21; border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.28); }
    h1 { margin: 0 0 10px; font-size: 1.8rem; color: #f3f4f6; }
    p { margin: 0; color: #9ca3af; line-height: 1.6; }
    .note { margin-top: 14px; padding: 12px 14px; border-radius: 12px; background: rgba(251,191,36,0.08); border: 1px solid rgba(251,191,36,0.25); color: #fcd34d; font-size: 0.95rem; }
    .rows { display: grid; gap: 12px; }
    .row { display: grid; grid-template-columns: minmax(0, 1fr) 180px 120px; gap: 12px; align-items: end; }
    label { display: block; font-size: 0.82rem; font-weight: 700; color: #cbd5e1; margin-bottom: 6px; }
    input, textarea, button { font: inherit; }
    input, textarea { width: 100%; border: 1px solid rgba(255,255,255,0.12); background: #0f131a; color: #f3f4f6; border-radius: 12px; padding: 12px 14px; outline: none; transition: border-color 0.2s ease, box-shadow 0.2s ease; }
    input:focus, textarea:focus { border-color: #60a5fa; box-shadow: 0 0 0 3px rgba(96,165,250,0.15); }
    textarea { min-height: 280px; resize: vertical; font-family: Consolas, "Courier New", monospace; line-height: 1.5; }
    .actions { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 16px; }
    button { border: none; border-radius: 12px; padding: 11px 16px; cursor: pointer; font-weight: 700; transition: transform 0.15s ease; }
    ...
    .status { margin-top: 14px; min-height: 24px; color: #93c5fd; font-size: 0.92rem; }
    @media (max-width: 820px) {
      body { padding: 16px; }
      .row { grid-template-columns: 1fr; }
      .btn-remove { width: auto; }
    }
  </style>
</head>
<body>
  <div class="wrap">
    <section class="card">
      <h1>${ui.title}</h1>
      <p>${ui.subtitle}</p>
      <div class="note">${ui.hint}</div>
    </section>
    <section class="card">
      <div id="rows" class="rows"></div>
      <div class="actions">
        <button class="btn-secondary" id="add-row-btn">${ui.addRow}</button>
        <button class="btn-secondary" id="load-example-btn">${ui.example}</button>
        <button class="btn-danger" id="clear-btn">${ui.clear}</button>
        <button class="btn-primary" id="generate-btn">${ui.generate}</button>
        <button class="btn-primary" id="copy-btn">${ui.copy}</button>
      </div>
      <div class="status" id="status"></div>
    </section>
    <section class="card">
      <label for="output">${ui.output}</label>
      <textarea id="output" readonly></textarea>
    </section>
  </div>
  <script>
    const UI = ${uiJson};
    const STORAGE_KEY = 'trprp_announcement_local_draft';
    ...
    function saveDraft() {
      const data = Array.from(document.querySelectorAll('.row')).map(row => ({
        ...
        soundId: row.querySelector('.sound-input').value
      }));
      localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
    }

    function addRow(name = '', soundId = '') {
      const row = document.createElement('div');
      row.className = 'row';
      row.innerHTML = \`
        <div>
          <label>\${UI.name}</label>
          <input class="name-input" type="text" placeholder="\${UI.namePlaceholder}" value="\${String(name).replace(/"/g, '&quot;')}">
        </div>
        <div>
          <label>\${UI.sound}</label>
          <input class="sound-input" type="number" min="0" step="1" placeholder="\${UI.soundPlaceholder}" value="\${String(soundId).replace(/"/g, '&quot;')}">
        </div>
        <div>
          <button class="btn-danger btn-remove" type="button">\${UI.rowRemove}</button>
        </div>\`;
    ...
    function loadDraft() {
      try {
        ...
        if (Array.isArray(saved) && saved.length) {
          saved.forEach(item => addRow(item.name || '', item.soundId || ''));
          return;
        }
      } catch (e) {}
      addRow();
    ...
    function loadExample() {
      ...
      ].forEach(([name, soundId]) => addRow(name, soundId));
      saveDraft();
      renderOutput(true);
    }
    ...
    function collectRows() {
      ...
      for (const row of rows) {
        ...
        const soundRaw = row.querySelector('.sound-input').value.trim();

        if (!name && !soundRaw) continue;

        const soundId = Number(soundRaw);
        if (!name || !soundRaw || !Number.isInteger(soundId) || soundId < 0) return null;

        result[name] = soundId;
        hasValid = true;
      }

      return hasValid ? result : {};
    }

    function renderOutput(showErrors) {
      const result = collectRows();
      if (result === null) {
        if (showErrors) setStatus(UI.invalidState);
        return false;
      }

      if (!Object.keys(result).length) {
        outputEl.value = '';
        if (showErrors) setStatus(UI.emptyState);
        return false;
      }
    ...
    async function copyOutput() {
      if (!renderOutput(true)) return;
      try {
        await navigator.clipboard.writeText(outputEl.value);
        setStatus(UI.copied);
      } catch (e) {
        setStatus(UI.copyError);
      }
    }

    document.getElementById('add-row-btn').addEventListener('click', () => { addRow(); saveDraft(); });
    ...
    document.getElementById('generate-btn').addEventListener('click', () => renderOutput(true));
...
function initBanner() {
  ...
  const iconEl = banner ? banner.querySelector('.error-banner-icon') : null;
  const chevronEl = banner ? banner.querySelector('.error-banner-chevron') : null;
  if (!banner) return;
  ...
  if (localStorage.getItem('announcementBannerCollapsed') === 'true') {
  ...
  if (toggleBtn) {
    toggleBtn.onclick = function () {
      ...
      localStorage.setItem('announcementBannerCollapsed', banner.classList.contains('collapsed') ? 'true' : 'false');
    };
  }

  if (btnLocal) {
    btnLocal.onclick = function () {
      localStorage.setItem('announcementGeneratorMode', 'local');
      localStorage.setItem('announcementBannerCollapsed', 'false');
      banner.classList.add('error-banner--local');
      banner.classList.remove('collapsed');
      if (titleEl) titleEl.textContent = t('banner-title-local');
      btnLocal.style.display = 'none';
      btnServer.style.display = '';
      renderLocalAnnouncement(localStorage.getItem('language') || 'ru');
    };
  }

  if (btnServer) {
    btnServer.onclick = function () {
      localStorage.setItem('announcementGeneratorMode', 'server');
      banner.classList.remove('error-banner--local');
      if (titleEl) titleEl.textContent = t('banner-title');
